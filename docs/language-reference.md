...
`ord`/`chr` operate on one byte (`0..255`). `format` supports a minimal int subset (`''`, `d`, width, zero-pad such as `04d`). `maketrans` builds a translation `dict` for `str.translate`. `ascii` escapes bytes `>= 128` as `\xHH`. `sum(iterable[, start])` adds list/tuple numbers with optional numeric `start` (default `0`); float promotes like `+` (D-0041). `iter(x)` / `next(it[, default])` use the same `Py68Range` cursor as `for`; exhaustion raises catchable `StopIteration` (D-0042).
